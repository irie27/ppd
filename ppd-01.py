...
server_type_var = None # To store "remote" or "local"
remote_plex_url_var = None
local_plex_address_var = None
plex_token_var = None
save_token_var = None # BooleanVar for the checkbox
...
all_music_playlists_global = [] # To store actual playlist objects
all_video_playlists_global = [] # To store actual playlist objects
...
# GUI elements for URL entries to enable/disable them
...
try:
    ...
    print(f"Critical libraries (plexapi, cloudscraper, requests) could not be imported: {e}")
    try:
        temp_root = tk.Tk(); temp_root.withdraw()
        messagebox.showerror("Critical Error", f"Required libraries are missing: {e}.\nPlease install plexapi, cloudscraper, and requests, then restart the application.")
        temp_root.destroy()
    except tk.TclError:
        pass
    sys.exit(1)
