...
def _common_download_loop(session, stream_url, full_filepath, display_filename, plex_server_token, connect_timeout, read_timeout):
...
def _download_remote_file_with_progress(plex_server_token, stream_url, full_filepath, display_filename):
    """Handles downloading a file from a remote server with specific retry logic."""
    global root_window, stop_download_event
    current_retry_delay = REMOTE_INITIAL_RETRY_DELAY
    # Ensure plex_instance_global._session is used, which should be a cloudscraper session for remote
    session = plex_instance_global._session if plex_instance_global and hasattr(plex_instance_global, '_session') else cloudscraper.create_scraper()


    for attempt in range(REMOTE_MAX_DOWNLOAD_RETRIES):
        if stop_download_event.is_set():
            log_status(f"    Download process stopped before attempting {display_filename}.")
            return False
        try:
            log_status(f"    Starting REMOTE download for: {display_filename} (Attempt {attempt + 1}/{REMOTE_MAX_DOWNLOAD_RETRIES})")
            return _common_download_loop(session, stream_url, full_filepath, display_filename, plex_server_token, REMOTE_CONNECT_TIMEOUT, REMOTE_READ_TIMEOUT)
            ...
            log_status(f"    REMOTE Download failed (HTTP {status_code}) for {display_filename} on attempt {attempt + 1}. Response: {response_text_snippet}")
            if 500 <= status_code < 600 and attempt < REMOTE_MAX_DOWNLOAD_RETRIES - 1:
                log_status(f"    Retrying in {current_retry_delay}s...")
                sleep_start_time = time.time()
                while time.time() - sleep_start_time < current_retry_delay:
                    if stop_download_event.is_set(): log_status(f"    Stop requested during retry delay for {display_filename}."); return False
                    time.sleep(0.1)
                current_retry_delay *= 2
                continue
            else: log_status(f"    Not retrying HTTP {status_code} or retries exhausted for {display_filename}."); break
        except requests.exceptions.RequestException as e:
            log_status(f"    REMOTE Download failed (RequestException: {e}) for {display_filename} on attempt {attempt + 1}.")
            if attempt < REMOTE_MAX_DOWNLOAD_RETRIES - 1:
                log_status(f"    Retrying in {current_retry_delay}s...")
                sleep_start_time = time.time()
                while time.time() - sleep_start_time < current_retry_delay:
                    if stop_download_event.is_set(): log_status(f"    Stop requested during retry delay for {display_filename}."); return False
                ...
                current_retry_delay *= 2
                continue
            else: log_status(f"    Retries exhausted for {display_filename} after RequestException."); break
        except IOError as e: log_status(f"    REMOTE Download failed (IOError: {e}) for {display_filename}. Not retrying."); break
        except Exception as e: log_status(f"    REMOTE Download failed (Unexpected Error: {e}) for {display_filename}. Not retrying."); import traceback; traceback.print_exc(); break
...
def _download_local_file_with_progress(plex_server_token, stream_url, full_filepath, display_filename):
    """Handles downloading a file from a local server with specific retry logic."""
    global root_window, stop_download_event
    current_retry_delay = LOCAL_INITIAL_RETRY_DELAY
    # Ensure plex_instance_global._session is used, which should be a standard requests session for local
    ...
    for attempt in range(LOCAL_MAX_DOWNLOAD_RETRIES):
        if stop_download_event.is_set():
            log_status(f"    Download process stopped before attempting {display_filename}.")
            return False
        try:
            log_status(f"    Starting LOCAL download for: {display_filename} (Attempt {attempt + 1}/{LOCAL_MAX_DOWNLOAD_RETRIES})")
            return _common_download_loop(session, stream_url, full_filepath, display_filename, plex_server_token, LOCAL_CONNECT_TIMEOUT, LOCAL_READ_TIMEOUT)
        except InterruptedError: return False
        except requests.exceptions.HTTPError as http_err:
            status_code = http_err.response.status_code
            log_status(f"    LOCAL Download failed (HTTP {status_code}) for {display_filename} on attempt {attempt + 1}.")
            if 500 <= status_code < 600 and attempt < LOCAL_MAX_DOWNLOAD_RETRIES - 1 :
                log_status(f"    Retrying in {current_retry_delay}s...")
                sleep_start_time = time.time()
                while time.time() - sleep_start_time < current_retry_delay:
                    if stop_download_event.is_set(): log_status(f"    Stop requested during retry delay for {display_filename}."); return False
            ...
            log_status(f"    LOCAL Download failed (RequestException: {e}) for {display_filename} on attempt {attempt + 1}.")
            if attempt < LOCAL_MAX_DOWNLOAD_RETRIES - 1:
                log_status(f"    Retrying in {current_retry_delay}s...")
                sleep_start_time = time.time()
                while time.time() - sleep_start_time < current_retry_delay:
                    if stop_download_event.is_set(): log_status(f"    Stop requested during retry delay for {display_filename}."); return False
        ...
        except IOError as e: log_status(f"    LOCAL Download failed (IOError: {e}) for {display_filename}. Not retrying."); break
        except Exception as e: log_status(f"    LOCAL Download failed (Unexpected Error: {e}) for {display_filename}. Not retrying."); import traceback; traceback.print_exc(); break
                ...
                if os.path.exists(filepath):
                    ...
                    try:
                        ...
                        if server_type_var.get() == "remote":
                            log_status(f"    Attempting remote download for {output_filename} using session type: {type(plex_instance_global._session)}")
                            download_successful = _download_remote_file_with_progress(plex_instance_global._token, stream_url, filepath, display_filename_short)
                        else: # local
                            log_status(f"    Attempting local download for {output_filename} using session type: {type(plex_instance_global._session)}")
                            download_successful = _download_local_file_with_progress(plex_instance_global._token, stream_url, filepath, display_filename_short)
